...
async def main():
    ...
    async with AsyncClient() as h:
        ...
        async for item in table.query(
            ...
            filter_expression=F("play_count").does_not_exist(),
            projection=F("pk") & F("sk") & F("updated_ts"),
        ):
            print(f'item sk={item['sk']} does not have play_count')
            items.append(item)
        ...
        transaction_items = [
            Update(
                ...
                expression=F("play_count").set(0),
                condition=(
                    F("play_count").does_not_exist()
                    & F("updated_ts").equals(item["updated_ts"])
                ),
            )
            for item in items
        ]
